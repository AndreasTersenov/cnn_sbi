...
def main():
    ...
    ranges = np.load(BOTH_CACHE + "/flat_local_ranges.npy")          # (16, 2) calibrated, frozen
    assert ranges.shape == (16, 2), ranges.shape
    stats = WLStatistics(n_scales=NS, device=dev, pixel_arcmin=RESO, dtype=torch.float64)
    sig, names, _ = fxl.select_frozen_sigma(SIG, "both", NB, dev)
    files = sorted(glob.glob(FC + "/cosmo_fiducial_perm*.npz"),
                   key=lambda f: int(f.split("perm")[-1].split(".")[0]))
    print(f"############ precompute fiducial 'both' datavectors ############")
    print(f"  {len(files)} perm files, ranges from {BOTH_CACHE}")
    ...
    for i, f in enumerate(files):
        ...
        x = fxl.build_and_l1(autos, "both", sig, stats, L1N, ranges, clamp_overflow=True)  # (180,3200)
        X.append(x)
    ...
    np.savez(OUT, x=X, perm=perms, patch=patches, truth=truth,
             channel_names=np.array(names), n_scales=NS, l1_nbins=L1N,
             ranges=ranges, note="both(16ch) flat-local obs datavectors; slice per arm via op_feature_columns")
    print(f"  saved {X.shape} -> {OUT}  ({time.time()-t0:.0f}s); truth={truth}")
