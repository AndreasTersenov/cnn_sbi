...
parser.add_argument("--map_kind", type=str, default="nbody")
...
parser.add_argument("--output_dim", type=int, default=12, help="Number of output features for compression")

args = parser.parse_args()
sigma_e = args.sigma_e
bin_number = args.bin
output_dim = args.output_dim  # summary (compressed) dimension
...
class Flow_nd_Compressor(hk.Module):
    def __call__(self, y):
        # Flow outputs a distribution over theta (param_dim), conditioned on summaries y
        nvp = NF_compressor(param_dim)(y)
        return nvp
...
compressor_eval = hk.transform_with_state(lambda y: CompressorCNN2D(output_dim)(y))
...
# init nf
params_nf = nf.init(
    jax.random.PRNGKey(0), theta=0.5 * jnp.ones([1, param_dim]), y=0.5 * jnp.ones([1, output_dim])
)
...
for batch in tqdm(range(1, args.total_steps + 1)):
    ...
    if batch % 2000 == 0:
        # save params
        with open(
            f"{save_dir}/params_nd_compressor_batch{batch}_{feature_tag}.pkl",
            "wb",
        ) as fp:
            pickle.dump(parameters_compressor, fp)

        with open(
            f"{save_dir}/opt_state_resnet_batch{batch}_{feature_tag}.pkl",
            "wb",
        ) as fp:
        ...
        plt.savefig(f"{fig_dir}/loss_compressor_{feature_tag}")
        plt.close()
        ...
        jnp.save(
            f"{save_dir}/loss_train_{feature_tag}.npy", loss_train
        )
        jnp.save(f"{save_dir}/loss_test_{feature_tag}.npy", loss_test)
        ...
        plt.legend()
        plt.title("Batch Loss")
        plt.savefig(f"{fig_dir}/loss_compressor_train_test_{feature_tag}")
        plt.close()

        # save contour plot, only if we are compressing to 6D
        if True:  # Always sample posterior over the 6 cosmological parameters
            ...
            sample_nd = nvp_sample_nd.apply(
                ...
                x=jnp.tile(y, (100000, 1)),  # repeat summary for conditional samples
            )
            ...
            plt.savefig(
                f"{fig_dir}/contour_plot_compressor_batch{batch}_{feature_tag}"
            )
            plt.close()
