#!/usr/bin/env python3
"""SBC rank test for the flat-local CNN arms (sibling of compute_sbc_from_tarp_dumps.py).

Identical logic; only the gate_c paths are repointed to the CNN tree (cnn_phase/gate_c).
Reuses the CNN GATE-C TARP dump posteriors. CPU-only.
"""
...
G = HERE + "/results/exploratory/flatsky_cross_2026_06/cnn_phase/gate_c/tarp_drp"
OUT = HERE + "/results/exploratory/flatsky_cross_2026_06/cnn_phase/gate_c/sbc"
...
def main():
    ...
    for ai, (arm, lab, col) in enumerate(ARMS):
        ...
        if not bs:
            print(f"{lab:10s}  (no dumps found at {G}/dumps/{arm}_*)")
            continue
        R = np.concatenate(list(bs.values()), axis=0)
        summary[arm] = {}
        for k, pi in enumerate(SCI):
            r = R[:, pi]
            ks_seed = [stats.kstest(bs[s][:, pi], "uniform").pvalue for s in bs]
            ...
            summary[arm][PNAMES[pi]]["bins_outside_99band"] = int(((dens < lo) | (dens > hi)).sum())
            if ai == 0:
    ...
    fig.suptitle("GATE C (CNN) — SBC rank histograms (flat within grey 99% band = calibrated; bars=mean of 3 seeds)",
                 fontsize=13)
    fig.tight_layout()
    for ext in ("png", "pdf"):
