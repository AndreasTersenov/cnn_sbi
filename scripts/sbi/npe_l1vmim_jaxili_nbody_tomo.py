...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--figure-out", type=str, default="posterior_l1vmim_tomo.pdf")
    p.add_argument(
        "--cache-dir",
        type=str,
        default=None,
    ...
    p.add_argument(
        ...
        type=float,
        default=1e-4,
        help="jaxili learning rate",
    )
    p.add_argument(
        "--nan-retries",
        type=int,
        default=10,
        help="Max retries on NaN losses during jaxili training",
    )
    p.add_argument(
        "--npe-warmup-steps",
        type=int,
    ...
    p.add_argument(
        "--checkpoint-name",
        type=str,
        default="params_l1vmim_jaxili",
        help="Checkpoint basename saved under save_dir/l1_vmim_jaxili/<map-kind>",
    )
    p.add_argument(
    ...
    # Weights & Biases
    p.add_argument("--wandb-project", type=str, default="l1-vmim-jaxili-npe-tomo")
...
def main() -> None:
    ...
    split_seed = int(args.seed + 2_700_001)
    ...
    param_names = [r"\Omega_m", r"\sigma_8", r"w_0", r"h_0", r"n_s", r"\Omega_b"]
    save_path = Path(args.save_dir) / "l1_vmim_jaxili" / args.map_kind
    npe_summary_path = save_path / "npe_training_summary.json"
    ...
    feature_mask_path = save_path / "summary_feature_mask.npz"
    ...
    if not args.no_wandb:
        ...
        wandb.define_metric("npe/epoch")
        wandb.define_metric("npe/*", step_metric="npe/epoch")
        wandb.define_metric("train/*", step_metric="npe/epoch")
        wandb.define_metric("val/*", step_metric="npe/epoch")
        wandb.config.update(
            {
                "run/method": "l1_vmim",
    ...
    feature_mask = np.ones(dataset_train_comp["x"].shape[1], dtype=bool)
    n_removed_features = 0
    if args.no_train:
        if feature_mask_path.exists():
            feature_data = np.load(feature_mask_path)
            feature_mask = np.asarray(feature_data["feature_mask"], dtype=bool)
            if feature_mask.shape[0] != dataset_train_comp["x"].shape[1]:
                raise ValueError(
                    "Saved feature mask dimension mismatch: "
                    f"{feature_mask.shape[0]} vs {dataset_train_comp['x'].shape[1]}"
                )
            n_removed_features = int(feature_mask.size - np.sum(feature_mask))
            if n_removed_features > 0:
                dataset_train_comp["x"] = dataset_train_comp["x"][:, feature_mask]
                dataset_val_comp["x"] = dataset_val_comp["x"][:, feature_mask]
                obs_comp = obs_comp[feature_mask]
            print(
                f"  Loaded feature mask from {feature_mask_path} "
                f"(removed {n_removed_features} zero-variance features)."
            )
    else:
        feature_mask, n_removed_features = filter_zero_variance_bins(
            dataset_train_comp["x"],
        ...
        if n_removed_features > 0:
            ...
            np.savez(
                ...
                min_variance=float(args.min_feature_variance),
            )
            print(f"  Saved feature mask -> {feature_mask_path}")
    ...
    wandb.log(
        {
            ...
            "data/removed_zero_variance_features": int(n_removed_features),
    ...
    # 6) Train or load jaxili NPE
    save_path.mkdir(parents=True, exist_ok=True)
    checkpoint_path = save_path / args.checkpoint_name
    theta_train = jnp.asarray(dataset_train_comp["theta"])
    x_train = jnp.asarray(dataset_train_comp["x"])
    metrics = None
    ...
    if args.no_train:
        try:
            ...
            exmp_input = (
                ...
                jnp.zeros((1, int(dataset_train_comp["x"].shape[1])), dtype=jnp.float32),
            )
        ...
        npe_summary_path.write_text(
            json.dumps(training_summary, indent=2),
            encoding="utf-8",
        )
        print(f"  Saved training summary to {npe_summary_path}")
        ...
        if standardization_applied and standardization_mean is not None and standardization_std is not None:
            ...
            print(f"  Saved summary standardization stats -> {summary_stats_path}")
    ...
    if not args.no_sample:
        ...
        posterior_samples = posterior.sample(
            x=jnp.asarray(obs_comp),
            num_samples=args.npe_samples,
            key=sample_key,
        )
        posterior_samples = np.asarray(posterior_samples)
        finite_mask = np.all(np.isfinite(posterior_samples), axis=1)
        posterior_samples = posterior_samples[finite_mask]
        ...
        metadata = {
            ...
            "feature_mask_source": (
                str(feature_mask_path.resolve()) if feature_mask_path.exists() else None
            ),
            "feature_mask_removed_count": int(n_removed_features),
            "compressor_source": compressor_source,
            ...
            "npe_samples_finite": int(posterior_samples.shape[0]),
            ...
            "diagnostic_files": {
                "npe_training_summary_path": (
                    str(npe_summary_path.resolve()) if npe_summary_path.exists() else None
                ),
        ...
        if npe_summary_path.exists():
            metadata["npe_training_summary"] = json.loads(
                npe_summary_path.read_text(encoding="utf-8")
            )
        if compressor_summary_path is not None and compressor_summary_path.exists():
        ...
        wandb.log(
            {
                "posterior/n_samples_finite": int(posterior_samples.shape[0]),
