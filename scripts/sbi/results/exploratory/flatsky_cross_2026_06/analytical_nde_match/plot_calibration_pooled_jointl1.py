#!/usr/bin/env python3
"""POOLED calibration overlay (no tercile split), 3 arms: l1+product / joint l1 / CNN ResNet18,
all VMIM->sbi_lens RealNVP. Pools every gate dump (tercile+seed) per arm — same pooling the
verdict uses — then (a) one TARP-DRP coverage curve per arm; (b) SBC rank histogram per param.
CPU-only (reads dumps). Outputs: tarp_pooled_jointl1_3arm.{png,pdf}, sbc_pooled_jointl1_3arm.{png,pdf}
"""
import glob
import matplotlib
matplotlib.use("Agg")
...
ARMS = {
    ...
    "joint l1 → RealNVP": dict(color="#2ca02c",
        dumps=f"{HERE}/jointl1_nobnt/gate/tarp_drp/dumps/jointl1_nobnt_*/seed_*/n*_m*/posterior_samples.npz"),
...
for i, (name, cfg) in enumerate(ARMS.items()):
    ...
    ax.fill_between(alpha, lo, hi, color=cfg["color"], alpha=0.20, zorder=2)
    worst = float(m[np.argmax(np.abs(m - alpha))] - alpha[np.argmax(np.abs(m - alpha))])
    net = float(np.trapz(m - alpha, alpha) * 2)
    ax.text(0.04, 0.94 - 0.055 * i, f"worst dev {worst:+.3f} | net {net:+.3f}",
...
ax.set_title("TARP-DRP coverage (pooled over terciles+seeds): l1+product vs joint l1 vs CNN\n"
             "on diagonal = calibrated · below = over-confident", fontsize=10)
fig.tight_layout()
for ext in ("png", "pdf"):
    fig.savefig(HERE / f"tarp_pooled_jointl1_3arm.{ext}", dpi=150, bbox_inches="tight")
print("wrote tarp_pooled_jointl1_3arm.png")
plt.close(fig)
...
fig.suptitle("SBC rank histograms (pooled over terciles+seeds): l1+product vs joint l1 vs CNN\n"
             "flat = calibrated · U-shape = over-confident · uniform rank-std = 0.289", fontsize=10)
fig.tight_layout(rect=[0, 0, 1, 0.92])
for ext in ("png", "pdf"):
    fig.savefig(HERE / f"sbc_pooled_jointl1_3arm.{ext}", dpi=150, bbox_inches="tight")
print("wrote sbc_pooled_jointl1_3arm.png")
