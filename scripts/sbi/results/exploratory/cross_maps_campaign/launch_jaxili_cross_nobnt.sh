#!/usr/bin/env bash
# Auto + cross L1-norms, NO BNT, zero-mean-maps, jaxili NPE.
#
# Same reference config as launch_jaxili_auto_nobnt.sh (lr=1e-4, batch 256,
...
#                                 : multipatch dataset
#
# 3 seeds, GPU 2. Cache shared across seeds (shape-noise re-drawn per pass
# but cached deterministically, matching the pattern of the auto-only runs).
set -euo pipefail

cd /mnt/home/tersenov/software/cnn_sbi

ROOT=scripts/sbi/results/exploratory/cross_maps_campaign/jaxili_cross_nobnt
...
for s in 41 42 43; do
  echo "===== JAXILI CROSS NOBNT seed ${s} (started $(date '+%Y-%m-%d %H:%M:%S')) ====="
  conda run --no-capture-output -n jaxili python -u \
    ...
    --posterior-out "$ROOT/posteriors/l1cross_tomo4_20deg160mp_nobnt_s${s}.npy" \
    --figure-out    "$ROOT/figures/l1cross_tomo4_20deg160mp_nobnt_s${s}.pdf"
  echo "===== JAXILI CROSS NOBNT seed ${s} (finished $(date '+%Y-%m-%d %H:%M:%S')) ====="
done
echo "===== JAXILI CROSS NOBNT ALL SEEDS DONE ($(date '+%Y-%m-%d %H:%M:%S')) ====="
