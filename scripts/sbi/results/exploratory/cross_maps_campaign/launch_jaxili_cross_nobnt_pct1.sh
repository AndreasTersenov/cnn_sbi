#!/usr/bin/env bash
# Auto + cross L1-norms, NO BNT, zero-mean-maps, jaxili NPE — pct=1 SNR knob.
#
# Same config as launch_jaxili_cross_nobnt.sh, except:
#   --cross-snr-percentile 1.0
# Cache lives under jaxili_cross_nobnt_pct1/ (separate from min/max-calibrated
# cache under jaxili_cross_nobnt/).
#
# 3 seeds, GPU 2. First seed builds the L1 cache; subsequent seeds reuse it.
set -euo pipefail
...
for s in 41 42 43; do
  echo "===== JAXILI CROSS NOBNT pct1 seed ${s} (started $(date '+%Y-%m-%d %H:%M:%S')) ====="
  conda run --no-capture-output -n jaxili python -u \
    ...
    --posterior-out "$ROOT/posteriors/l1cross_tomo4_20deg160mp_nobnt_p1_s${s}.npy" \
    --figure-out    "$ROOT/figures/l1cross_tomo4_20deg160mp_nobnt_p1_s${s}.pdf"
  echo "===== JAXILI CROSS NOBNT pct1 seed ${s} (finished $(date '+%Y-%m-%d %H:%M:%S')) ====="
done
echo "===== JAXILI CROSS NOBNT pct1 ALL SEEDS DONE ($(date '+%Y-%m-%d %H:%M:%S')) ====="
