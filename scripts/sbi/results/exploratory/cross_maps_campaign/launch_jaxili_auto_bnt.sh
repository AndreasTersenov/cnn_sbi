#!/usr/bin/env bash
# Auto-only, BNT, jaxili NPE — exact reference config (run_bnt_tomo4_study.py).
# 3 seeds on GPU 0. Cache shared across seeds.
set -euo pipefail

cd /mnt/home/tersenov/software/cnn_sbi

ROOT=scripts/sbi/results/exploratory/cross_maps_campaign/jaxili_auto_bnt
...
for s in 41 42 43; do
  echo "===== JAXILI AUTO BNT seed ${s} (started $(date '+%Y-%m-%d %H:%M:%S')) ====="
  conda run --no-capture-output -n jaxili python -u \
    scripts/sbi/npe_l1norm_jaxili_nbody_tomo.py "${COMMON[@]}" \
    --seed $s \
    --save-dir "$ROOT/seed_${s}" \
    --posterior-out "$ROOT/posteriors/l1_tomo4_20deg160_bnt_s${s}.npy" \
    --figure-out    "$ROOT/figures/l1_tomo4_20deg160_bnt_s${s}.pdf"
  echo "===== JAXILI AUTO BNT seed ${s} (finished $(date '+%Y-%m-%d %H:%M:%S')) ====="
done
echo "===== JAXILI AUTO BNT ALL SEEDS DONE ($(date '+%Y-%m-%d %H:%M:%S')) ====="
