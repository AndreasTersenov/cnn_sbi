#!/usr/bin/env bash
# Recalibration probe for the percentile-based cross SNR knob.
# Usage: calibrate_cross_snr_pct.sh <gpu> <pct> <tag>
#   gpu: CUDA_VISIBLE_DEVICES index
#   pct: --cross-snr-percentile value (e.g. 1.0 or 5.0)
#   tag: subdir suffix under jaxili_cross_bnt_pct${tag} (e.g. p1 or p5)
#
# Runs BNT seed 41 end-to-end so we get both the calibrated range AND a
# trained posterior under the new binning. Cache lives under a tag-specific
# directory to keep the original (min/max-calibrated) cache intact.
set -euo pipefail
...
PCT=$2
TAG=$3
ROOT=scripts/sbi/results/exploratory/cross_maps_campaign/jaxili_cross_bnt_pct${TAG}
...
conda run --no-capture-output -n jaxili python -u \
  ...
  --posterior-out "$ROOT/posteriors/l1cross_tomo4_20deg160mp_bnt_p${TAG}_s41.npy" \
  --figure-out    "$ROOT/figures/l1cross_tomo4_20deg160mp_bnt_p${TAG}_s41.pdf"
