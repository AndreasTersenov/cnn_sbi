#!/usr/bin/env python3
"""A4: ResNet-50 demeaned no-BNT CNN-VMIM capacity sweep.

Runs npe_cnn_nbody_tomo.py for every combination of
  cdim ∈ {10, 20, 50}  ×  seed ∈ {42, 43}

with --zero-mean-maps and compressor-arch=resnet50.  Jobs are dispatched in
parallel, one per GPU, cycling round-robin over the supplied GPU list.

Usage
-----
conda run -n jaxili python run_a4_resnet50_sweep.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="A4 ResNet-50 demeaned no-BNT capacity sweep.")
...
def build_job(
    ...
    tag = f"resnet50_cdim{cdim}_s{seed}"
    run_dir = output_root / tag
    log_dir = output_root / "logs"
    posterior_out = output_root / "posteriors" / f"cnn_resnet50_zm_nobnt_cdim{cdim}_s{seed}.npy"
    figure_out = output_root / "figures" / f"cnn_resnet50_zm_nobnt_cdim{cdim}_s{seed}.pdf"
...
def main() -> None:
    ...
    jobs = [build_job(args, cdim, seed, args.output_root) for cdim in cdims for seed in seeds]
    print(f"[A4] {len(jobs)} jobs, {len(gpus)} GPUs: {gpus}")
    for j in jobs:
        print(f"  {j.name}")
    ...
    print(f"\n[A4] Manifest written to {manifest_path}")
